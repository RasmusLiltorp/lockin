...
#qcard(
  ...
  answer: [*a.* $T(n) = Theta(n^(1.5) log n)$ — tilfælde 2. *b.* (i) tilfælde 1, (ii) tilfælde 2, $Theta(n log^2 n)$, (iii) tilfælde 1, (iv) tilfælde 3.],
  blueprint: [
    For hver ligning: regn $alpha$, hold $f$ op mod $n^alpha$, og afgør, om gabet er en hel potens eller bare en $log$-faktor.
    ...
    + *Log-faktorer er tilfælde 2.* Er $f = Theta(n^alpha log^k n)$ med $k >= 0$, er det tilfælde 2 med svaret $Theta(n^alpha log^(k+1) n)$ — 4. udgaves regel. (Gamle løsningsforslag kaldte dette et hul.)
  ],
  worked: [
    ...
    *b.* Sammenlign $f$ mod $n^alpha$ for hver:
    ...
    + $alpha = log_13 13 = 1$, så $n^alpha = n$. Her er $f = n log n = Theta(n^1 log^1 n)$, altså tilfælde 2 med $k = 1$. Svar: $Theta(n log^2 n)$. (Den gamle sætning kaldte dette et hul, fordi den manglede $k$-leddet.)
...
#qcard(
  ...
  answer: [(i) tilfælde 1, $Theta(n)$. (ii) tilfælde 2, $Theta(n log n)$. (iii) tilfælde 2, $Theta(n log^2 n)$. (iv) tilfælde 3, $Theta(n^2)$.],
  blueprint: [
    ...
    + *Vælg tilfælde.* Polynomielt mindre: tilfælde 1. Samme orden, evt. med en $log^k$-faktor ($k >= 0$): tilfælde 2 med $Theta(n^alpha log^(k+1) n)$. Polynomielt større (plus regularitet): tilfælde 3.
  ],
  worked: [
    ...
    + $f = n log n = Theta(n^1 log^1 n)$. Det er tilfælde 2 med $k = 1$. $T(n) = Theta(n log^2 n)$. (Med 3. udgaves sætning faldt dette i et hul; 4. udgaves $k$-led løser det.)
...
#qcard(
  ...
  answer: [(i) tilfælde 1, $Theta(n^(log_2 5))$. (ii) tilfælde 2, $Theta(n log^2 n)$. (iii) tilfælde 3, $Theta(sqrt(n))$. (iv) kan ikke løses (forkert form).],
  blueprint: [
    Tjek først formen, så regn $alpha$ og hold $f$ op mod $n^alpha$. Den ægte faldgrube her er den subtraktive form.
    ...
    + *Aflæs og regn $alpha$.* Find #swap[$a$], #swap[$b$], #swap[$f(n)$] og $alpha = log_b a$.
    + *Sammenlign.* Polynomielt mindre $f$ giver tilfælde 1; samme orden tilfælde 2; polynomielt større (plus regularitet) tilfælde 3.
    + *Log-faktorer er tilfælde 2.* Er $f = Theta(n^alpha log^k n)$ med $k >= 0$, er det tilfælde 2 med $Theta(n^alpha log^(k+1) n)$. (Gamle løsningsforslag kaldte dette et hul.)
  ],
  worked: [
    ...
    + $a = 5$, $b = 5$, $f(n) = n log n$. $alpha = log_5 5 = 1$, så $n^alpha = n$. Her er $f = n log n = Theta(n^1 log^1 n)$, altså tilfælde 2 med $k = 1$. $T(n) = Theta(n log^2 n)$. (Den gamle sætning kaldte dette et hul.)
